...
export default function Welcome() {
  ...
  const { data: me, isLoading, error } = useQuery({
    ...
    queryFn: fetchMe,
    enabled: !!user,
    refetchInterval: 15_000, // poll so users see approval without a manual refresh
  });
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[var(--surface-base,#f7f8fa)] px-4 py-8">
      <div className="w-full max-w-md rounded-lg border border-[var(--border-subtle,#e5e7eb)] bg-white p-8 shadow-sm">
        ...
        {isRejected ? (
          ...
          <>
            ...
            <div className="mt-6 border-t border-[var(--border-subtle,#e5e7eb)] pt-6">
              <h2 className="text-sm font-semibold text-[var(--text-primary,#111827)]">
                Fortell oss litt mer
              </h2>
              <p className="mt-1 text-xs text-[var(--text-muted,#6b7280)]">
              ...
              <form onSubmit={handleSave} className="mt-4 space-y-3">
                ...
                <button
                  ...
                  className="w-full rounded-md bg-[var(--text-primary,#111827)] px-4 py-2 text-sm font-medium text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
                >
        ...
        {isLoading && (
          <p className="mt-4 text-xs text-[var(--text-muted,#6b7280)]">Henter status...</p>
        )}
        {error && (
        ...
        <div className="mt-6 border-t border-[var(--border-subtle,#e5e7eb)] pt-4 text-center">
          <button
            type="button"
            onClick={() => signOut()}
            className="text-xs text-[var(--text-muted,#6b7280)] underline"
          >
            Logg ut
          </button>
        </div>
