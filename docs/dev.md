...
- [x] ~~**Treemap → viewer cross-filter**~~ — code-side fix landed across 2026-05-17. Type page (`/types`) wires the v3 isolation path via commit `6584bef`; Model Workspace (`/models/:modelId`) typeVisibility path was missing a case-tolerant lookup for `IFCWALL` ↔ `IfcWall` (fixed in `86d6756`), a culler-dirty kick after v3 `setVisible` (`6df92df`), and a render nudge so the canvas paints without a click-inside (`9cfa58e`). Verify on the live `shapes_probe.ifc` model: click Plate / Member / Annotation tiles; viewer should hide everything else.
- [x] ~~**Same-click toggles off**~~ — `ModelWorkspace.filterByQuality` now mirrors the `isSame ? clear : set` pattern used by treemap / storey / geometry. Type page tables call `handleToggleType` instead of `handleSelectType` so the active row clears on re-click. Other surfaces (TypeTreemap, TypeTopBarList, IfcClass chip, Materials family treemap, viewer pick) already had the toggle-off semantics. FilterChips body-click intentionally isolates (X removes, Reset clears all) — single producer with two affordances. Shipped 2026-05-17.
- [x] ~~**Counts agree across tiles**~~ — added a `"X types · Y instances"` subtitle on the Model dash + Type page viewer panes that animates with `useCountUp` and binds to the cross-filtered slice. `filterAnalysisTypes` now projects each type's `instance_count` down to its `storey_distribution[].instance_count` when a floor filter is active, so KPI cluster / treemap / quality / viewer subtitle all reflect what's actually visible on the storey. Shipped 2026-05-17.
- [x] ~~**Count-up animation everywhere**~~ — observed widely adopted in the 2026-05-17 audit (Type page, Model dash KPI cluster, Materials KPI, Project dash, Models gallery, Storey chart). Closed with the viewer-subtitle pass.
- [x] ~~**Persistent viewer everywhere**~~ — ViewerPane + isolation shipped on Type + Model dash (2026-05-15). Materials and Project dash don't host a `UnifiedBIMViewer` yet, so the item is moot until a viewer ships there.
- [x] ~~**Extractor must surface every class the viewer can render**~~ — IfcAnnotation now feeds the synthetic-type pipeline in `ifc_parser.py` (the untyped tracker iterates `IfcElement` AND `IfcAnnotation`). Annotations get treemap chips and the user can isolate/hide them via the standard filter store. Shipped 2026-05-17. (IfcSpace stays excluded — spatial container, not a chip.) Re-extract `shapes_probe.ifc` to verify.
- [x] ~~**Source-of-filter is highlighted, never self-filters**~~ — `TypeBrowserV2` and `ModelWorkspace` now build per-producer slices (`classUnfilteredTypes`, `storeyUnfilteredTypes`). Elements treemap and Storeys chart stay whole on their own dimension; active tile gets the highlighted state via `DrillTarget`. Materials family treemap already correct. Shipped 2026-05-17.
- [x] ~~**Cross-filter must be bidirectional**~~ — viewer pick now drives `ifc_class` on Model dash, Type page, and Federated viewer. Same toggle-off semantics as dashboard tiles; null/Unknown selections do not auto-clear. Shipped 2026-05-17. Follow-up: `type_guid` and `floor_code` dimensions also driven from viewer (requires instance → type / instance → storey resolution from the v3 worker payload).
- [x] ~~**Viewer requires click-inside before filters apply**~~ — hoisted `nudgeRender` callback in `UnifiedBIMViewer.tsx`; wired into typeVisibility effect, isolation isolate/highlight/restore branches, and the v2 synchronous fallback. Shipped 2026-05-17.
- [ ] **Info panel: instance-level data dominates; need per-level insight strategy** — the model + type viewer info panels are currently wired toward INSTANCE properties and largely duplicate each other. Strategy needed: distinct payload per interaction level.
  - **Class** (treemap chip clicked, no specific type/instance selected): classification distribution across types in the class, MMI histogram, material breakdown, count + reuse ratio.
  - **Type** (specific type selected): the type's classification, MMI, material layers, instance count, geometry quality summary, cross-project TypeBank refs.
  - **Instance** (specific element picked in viewer): geometry-specific properties, location, host element, attached Psets, linked annotations / issues.
  - All three accessible via viewer interaction AND via the data dashboards (treemap → class, table row → type, viewer click → instance). Eliminate the current property-bag duplication.
...
- **2026-05-18** — Materials Library 11× fan-out collapsed into a single `/api/types/types/project-materials/?project_id=…` call. Backend: slim grouped queryset over `TypeDefinitionLayer ↔ TypeMapping ↔ IFCType ↔ Model`, returns only the fields `aggregateProjectMaterials` consumes. Frontend: `useProjectMaterials` swapped from `useQueries` fan-out to a single `useQuery`. Query-count regression guard in `tests/unit/test_project_materials_endpoint.py`.
- **2026-05-17** — Cross-filter "feels right" second pass: render kick (no more click-inside before paint), source-of-filter highlight (Elements treemap + Storeys chart stay whole on own dimension), bidirectional cross-filter (viewer pick drives ifc_class on Model dash / Type page / Federated viewer), extractor surfaces IfcAnnotation as synthetic type. See `docs/worklog/2026-05-17-21-40_cross-filter-feels-right-second-pass.md`.
