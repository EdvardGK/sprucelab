...
- [ ] **Ghost objects — fragments-v3 collapses `IfcOpeningElement` extrusions to their bare profile.** Diagnosed 2026-05-16 via a clean-room probe (`docs/plans/`+ uploaded as `ghost_probe.ifc`): out of 14 IFC classes with identical `IfcExtrudedAreaSolid` representations, only the opening rendered as a 0-thickness plane that "cuts cleanly through the middle" of its bounding box. Test cubes (single + 10×10×10 BuildingElementProxy) had no ghosts at all → not a Clipper leak, not a converter-side default. The fragments converter loses the extrusion step on opening elements specifically. Fix paths: (a) backend extractor filters `IfcOpeningElement` out of the fragments group entirely (they're supposed to be Boolean-subtracted from hosts, not standalone), (b) frontend viewer hides them by default with an opt-in toggle, (c) fix the converter to actually sweep the profile. Backend filter is cheapest.
