...
- [x] ~~**Extractor surfaces every class except spatial containers and annotations**~~ — IfcElement-derived synthetic types feed the untyped pipeline; IfcSpace stays excluded (spatial container, not a chip); **IfcAnnotation was briefly included 2026-05-17 and walked back 2026-05-18** — annotations belong to the Annotation primitive in the spine (overlay on source data), not the IFC type extraction. Annotation views live alongside drawings + claims + log events as a separate surface. Re-extract any model previously containing annotation synthetic types to drop the stale rows.
