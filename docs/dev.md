...
- [ ] **Extractor must surface every class the viewer can render** — `backend/ifc-service/services/ifc_parser.py:171,256` filters `IfcAnnotation` and `IfcSpace` out of the types list, but `web-ifc` still tessellates IfcAnnotation when it has a 3D body. Result: a pink star renders in the viewer with no matching treemap chip — user can't filter to it. Either include them in the type extraction (preferred — they're real elements with geometry the user wants to control) or default-hide them in the viewer too. Inconsistency surfaced 2026-05-17 on `shapes_probe.ifc`.
...
- [ ] **Cross-filter must be bidirectional** — confirmed working 2026-05-17: treemap → viewer isolates correctly (commits `6584bef` + `86d6756` + `6df92df`). Reverse direction does NOT work: clicking / selecting an element in the viewer doesn't drive the treemap, KPI tiles, table, charts. Wire `UnifiedBIMViewer`'s `onSelectionChange` (already exists) to dispatch a filter mutation (or selection-state) through `useProjectFilterActions`; every consumer surface should react. Same pattern for storey bar clicks, KPI tile clicks — anything that can BE a filter source.
- [ ] **Viewer requires click-inside before filters apply** — observed 2026-05-17 after the v3 cross-filter fix landed. Clicking a treemap tile or storey bar updates the dashboard state but the viewer often doesn't redraw until the user clicks INSIDE the viewer canvas (or interacts with the camera). Worse when rapidly switching between filters. Hypothesis: the render loop is camera-event-driven (`controls.rest`, `camera.controls` listener) — visibility changes mark culler dirty but no camera event fires, so the next paint waits for user interaction. Fix: explicitly request a render frame (e.g. `worldRef.current.renderer.three.render(scene, camera)` or `world.update()`) after `cullerRef.needsUpdate = true`. Mirror the floor-filter effect's render kick.
