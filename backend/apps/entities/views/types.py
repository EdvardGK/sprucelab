...
class IFCTypeViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='project-materials')
    def project_materials(self, request):
        """
        Slim project-scoped feed for the Materials page aggregator.

        GET /api/types/project-materials/?project_id={id}

        Collapses the previous 11× fan-out (one
        ``/api/types/?model=…&page_size=10000&expand=mapping`` per ready
        model) into a single response. Returns only the fields
        ``aggregateProjectMaterials`` consumes; the heavy ``properties``
        blob and full ``TypeMapping`` are dropped.

        Cost: one DB roundtrip across the joined queryset
        ``TypeDefinitionLayer ↔ TypeMapping ↔ IFCType ↔ Model``, grouped
        per model in Python.
                  ...
                  "types": [
                    {
                      ...
                      "mapping": {
                        "definition_layers": [
                          {"layer_order": 1, "material_name": "...",
                           "ns3457_code": null, "quantity_per_unit": 0.25,
                           "material_unit": "m3", "thickness_mm": 250,
                           "epd_id": null}
                        ]
                      }
                    }
        ...
        Types without definition layers are omitted (the aggregator skips
        them anyway). ``instance_count > 0`` filter matches every other
        project-level types query in this viewset.
        """
        project_id = request.query_params.get('project_id')
        if not project_id:
            return Response(
        ...
        layers = (
            ...
            .filter(
                type_mapping__ifc_type__model__project_id=project_id,
                type_mapping__ifc_type__instance_count__gt=0,
            )
            .select_related(
                'type_mapping__ifc_type',
                'type_mapping__ifc_type__model',
            )
            .order_by(
                'type_mapping__ifc_type__model_id',
                'type_mapping__ifc_type_id',
                'layer_order',
            )
        )

        types_by_id: dict[str, dict] = {}
        type_order_by_model: dict[str, list[str]] = {}
        models_meta: dict[str, str | None] = {}

        for layer in layers:
            t = layer.type_mapping.ifc_type
            model = t.model
            model_id = str(model.id)
            type_id = str(t.id)

            if model_id not in models_meta:
                models_meta[model_id] = model.name
                type_order_by_model[model_id] = []

            type_entry = types_by_id.get(type_id)
            if type_entry is None:
                type_entry = {
                    ...
                    'mapping': {'definition_layers': []},
                    '_model_id': model_id,
                }
                types_by_id[type_id] = type_entry
                type_order_by_model[model_id].append(type_id)

            type_entry['mapping']['definition_layers'].append({
        ...
        models_payload = []
        for model_id, type_ids in type_order_by_model.items():
            types_payload = []
            for tid in type_ids:
                entry = types_by_id[tid]
                entry.pop('_model_id', None)
                types_payload.append(entry)
            models_payload.append({
                'model_id': model_id,
                'model_name': models_meta[model_id],
                'types': types_payload,
            })
